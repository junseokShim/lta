...
**문제**: chat 모드가 일반 클라우드 챗봇처럼 동작하여, `data 폴더 분석해줘`나 `README.md 요약해줘` 같은 요청에 "로컬 파일에 접근할 수 없습니다. 파일을 업로드해 주세요." 라고 잘못 응답했습니다.

**원인**: `chat_direct()` 메서드가 사용자 메시지만 LLM에 전달하고 어떠한 파일 내용도 읽지 않았습니다. LLM의 base 학습 데이터가 클라우드 환경을 전제로 하므로 파일 접근 불가라고 응답.

**수정**: chat 모드가 워크스페이스 파일을 직접 읽어 LLM에 전달하도록 개선했습니다.

#### chat 모드 워크스페이스 인식 동작

chat 모드는 이제 사용자 메시지를 분석하여 관련 파일/폴더를 자동으로 읽습니다:

1. **메시지에서 경로 힌트 추출** — `data/`, `README.md`, `config.py` 같은 파일/폴더 참조 감지
2. **워크스페이스 내 존재 확인** — 경로 탈출(../) 공격 방지, 워크스페이스 경계 내 경로만 허용
3. **파일/디렉토리 내용 수집** — 파일이면 내용 읽기, 디렉토리면 목록 나열
4. **researcher로 관련 파일 추가 탐색** — RAG 또는 키워드 기반 관련 파일 발견
5. **LLM 프롬프트에 삽입** — 실제 파일 내용을 포함한 컨텍스트로 LLM 호출

**접근 가능 범위**:
- 현재 활성 워크스페이스 루트(`workspace_root`) 하위의 모든 파일/폴더
- 경로 탈출(`../`) 시도는 자동 차단
- 이진 파일(PNG, 실행파일 등)은 메타정보만 제공
- 워크스페이스 외부 경로(`/etc/passwd` 등)는 접근 거부

**한국어 폴더 키워드 지원**: `데이터 폴더 → data/`, `소스 코드 → src/`, `설정 파일 → config/` 등

#### chat 모드 vs agent 모드

| 구분 | chat 모드 (기본) | agent 모드 |
|------|-----------------|-----------|
| 동작 방식 | 파일 직접 읽기 + LLM 직접 호출 | 전체 multi-agent 오케스트레이션 |
| 처리 속도 | 빠름 (단일 LLM 호출) | 느림 (Manager→Planner→Coder→Reviewer→Tester) |
| 파일 접근 | 읽기 전용 (워크스페이스 내) | 읽기+쓰기 (실제 파일 반영) |
| 적합한 요청 | 파일 분석/설명, 비교, 요약, 질문 | 코드 생성/수정, 테스트 작성, 프로젝트 실행/검증 |
...
```
/mode           현재 활성 모드 확인
/mode chat      chat 모드로 전환 (워크스페이스 인식 직접 대화)
/mode agent     agent 모드로 전환 (전체 오케스트레이션)
```

#### chat 모드에서 escalation

chat 모드에서 요청이 파일 생성·코드 수정 등 복잡한 작업을 포함하는 경우:
- LLM 응답 말미에 "이 작업은 agent 모드(/mode agent)에서 더 잘 처리할 수 있습니다." 안내 포함
- 키워드 기반 감지: "파일 생성", "구현해줘", "수정해줘", "implement", "create file" 등
